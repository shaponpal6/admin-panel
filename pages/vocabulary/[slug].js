        ...
        <>
          <section>
            ...
            {type==="lessons" ? (
              ...
              <div>
                <PostForm postRef={postRef} defaultValues={post} preview={true} />
              </div>
            )}
...
function PostForm({ defaultValues, postRef, preview=false }) {
  ...
  return (
    <form onSubmit={handleSubmit(updatePost)}>
      {!edit ? (
        <div onClick={()=>setEdit(true)} className={styles.button2} >
          <p style={{margin: 0}}>{title}</p>
        </div>
      ): <div onClick={()=>setEdit(false)} className={styles.button}>Hide</div>}
